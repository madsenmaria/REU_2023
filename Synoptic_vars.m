...
Geo500_final = Geo500_final(152:8631,:,:); 
u250_final = u250_final(152:8631,:,:); 

u250_t_final(leap_ind, :) = []; 
u250_t_final = u250_t_final(152:8631, :); 
...
hail = hail(:,:,152:8631); 
sig_hail = sig_hail(:,:,152:8631); 
tor = tor(:,:,152:8631); 
sig_tor = sig_tor(:,:,152:8631); 
wind = wind(:,:,152:8631); 
sig_wind = sig_wind(:,:,152:8631); 

olr_short = olr_short(:,:,152:8631); 

%% Reshape and calculate daily mean 
z_byyear = reshape(Geo500_final, [212 40 51 241]);    % now shaped as day, year, lat, lon
u_byyear = reshape(u250_final, [212 40 51 241]); 
...
z_season = zeros(212, 40, 51, 241);
u_season = zeros(212, 40, 51, 241);
...
z500 = zeros(212, 40, 51, 241);
u250 = zeros(212, 40, 51, 241);
for i = 1:40
...
olr_byyear = reshape(olr_short, [212 40 13 144]); 
...
olr_season = zeros(212, 40, 13, 144);
...
z500 = reshape(z500, [8480 51 241]); 
u250 = reshape(u250, [8480 51 241]); 
olr = olr_short;  
